...
plot_metrics_D_reg <- function(pseudotime,
  ...
  plot(pseudotime, D_res$s_i_naive,
       ...
       main = sprintf("Naïve (ρ = %.2f, D = %.2f)",
                      ...
                      D_res$D_naive))
  if (smooth == "lm") abline(lm(D_res$s_i_naive ~ pseudotime), col = col_naive, lwd = 2)
  if (smooth == "loess") {
    fit <- loess(D_res$s_i_naive ~ pseudotime)
    lines(sort(pseudotime), predict(fit, newdata = data.frame(pseudotime = sort(pseudotime))),
          col = col_naive, lwd = 2)
  }
  ...
  plot(pseudotime, D_res$s_i_term,
       ...
       main = sprintf("Terminal (ρ = %.2f, D = %.2f)",
                      ...
                      D_res$D_term))
  if (smooth == "lm") abline(lm(D_res$s_i_term ~ pseudotime), col = col_term, lwd = 2)
  if (smooth == "loess") {
    fit <- loess(D_res$s_i_term ~ pseudotime)
    lines(sort(pseudotime), predict(fit, newdata = data.frame(pseudotime = sort(pseudotime))),
          col = col_term, lwd = 2)
  }
}

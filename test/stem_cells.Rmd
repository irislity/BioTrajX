...
p <- DimPlot(seu, reduction="umap", group.by="Phenotype") +
          DimPlot(res$obj, reduction="umap",group.by="Phenotype")
...
expr <- as.matrix(GetAssayData(res$obj, layer = "data"))

# Compare all methods
results <- compute_multi_DOPE(
  expr = expr,
...
```
```{r}
plot.multi_dope_results (res_multi, type="bar")
```
